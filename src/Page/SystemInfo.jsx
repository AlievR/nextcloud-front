...
function SystemInfo(props) {
    ...
    useEffect(async () => {
        try {
            const system_data = await request(`/api/systems/${id}`, 'GET')
    ...
    useEffect(async () => {
        try {
            const data = await request(`/api/uploads/${id}`, 'GET')
            setIsLoaded(true);
            setItems(data);
        } catch (error) {
            setIsLoaded(true);
    ...
    const uploadScheme = async (selectedFile) => {
        try {
            ...
            const response = await axios.post(`/api/uploads/${id}`, data)
    ...
    const removeScheme = async (_id, fileSrc, name) => {
        try {
            const data = await axios.delete(`/api/uploads/schema/${_id}`, {
                data: {
                    fileSrc
                }
            })
    ...
    const editScheme = async (_id, name) => {
        try {
            const data = await axios.put(`/api/uploads/schema/${_id}`, { name })
