...
function SystemList() {
    ...
    useEffect(async () => {
        try {
            const data = await request('/api/systems', 'GET')
            setIsLoaded(true);
            setItems(data);
        } catch (error) {
            setIsLoaded(true);
    ...
    const addSystem = async (request_type, formInfo) => {
        try {
            const data = await request('/api/systems', String(request_type), formInfo)
    ...
    const removeScheme = async (_id, request_type) => {
        try {
            const data = await request('/api/systems', String(request_type), { _id })
            toast.success(data.message)
            setIsLoaded(false)
        } catch (error) {
            toast.error(error)
    ...
    const editScheme = async (_id, name) => {
        try {
            const data = await axios.put(`/api/systems/${_id}`, { name })
